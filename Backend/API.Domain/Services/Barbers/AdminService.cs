...
{
    public class AdminService : BasicService<Admin, AdminValidator>, IAdminService
    {

        public AdminService(IUnitOfWork<Admin> repositorios, IHttpContextAccessor httpContext) : base(repositorios, httpContext)
        {
        }
        ...
        public async Task<decimal> TotalIngresosMes()
        {
            var ahora = DateTime.UtcNow;

            // Suma de precios de suscripciones activas de barberos
            var ingresosBarberos = await _repositorios.Barberos
                .GetQuery()
                .AsNoTracking()
                .Where(b => b.EstadoSuscripcion == EstadoSuscripcion.Aprobada &&
                            b.FechaVencimientoSuscripcion >= ahora)
                .Select(b => b.Suscripcion != null ? b.Suscripcion.Precio ?? 0 : 0)
                .SumAsync();

            // Suma de precios de suscripciones activas de barberías
            var ingresosBarberias = await _repositorios.Barberias
                .GetQuery()
                .AsNoTracking()
                .Where(ba => ba.EstadoSuscripcion == EstadoSuscripcion.Aprobada &&
                             ba.FechaVencimientoSuscripcion >= ahora)
                .Select(ba => ba.Suscripcion != null ? ba.Suscripcion.Precio ?? 0 : 0)
                .SumAsync();

            return ingresosBarberos + ingresosBarberias;
        }
        ...
        public async Task<DatosDashboardAdminDto> ObtenerDatosDashBoardAdmin()
        {
            var totalBarberos = await TotalBarberos();
            var totalBarberias = await TotalBarberias();
            var totalIngresosMes = await TotalIngresosMes();
            ...
            return new DatosDashboardAdminDto
            {
        ...
        public async Task<List<DatosBarberiasBarberosAdminDto>> ObtenerBarberiasYBarberos()
        {
            List<DatosBarberiasBarberosAdminDto> listaRetorno = new();
            ...
            var barberias = await _repositorios.Barberias
                                    ...
                                    .Select(e => new DatosBarberiasBarberosAdminDto
                                    {
