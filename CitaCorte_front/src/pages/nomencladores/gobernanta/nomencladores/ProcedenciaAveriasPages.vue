<template>
    <div class="q-pa-xl">
        <q-breadcrumbs class="qb cursor-pointer q-pb-md">
            ...
            <q-breadcrumbs-el label="Procedencia Averia" />
        </q-breadcrumbs>
            ...
            <header class="q-pa-sm bg-primary">
                <q-toolbar>
                    <q-toolbar-title class="text-subtitle6 text-white">
                        {{
                            objeto?.id ? "Editar Procedencia Averia" : "Adicionar Procedencia Averia"
                        }}</q-toolbar-title>
                </q-toolbar>
            </header>
...
<script setup>
// IMPORT LIBRERIES
import { dataColumnProcedenciaAveria } from "src/assets/js/column_data/columnDataGobernanta";
import {
    loadGet,
    saveData,
    isValorRepetido,
...
// DATOS DE LA TABLA
const columns = dataColumnProcedenciaAveria;
const filter = ref("");
const items = ref([]);
...
onMounted(async () => {
    ...
    items.value = (await loadGet('ProcedenciaAverias/ObtenerListadoPaginado')) ?? []; // Condicion para en caso de error la tabla no de error ya q la api devulve undefined
...
const Guardar = () => {
    const url = objeto.id ? 'ProcedenciaAverias/Actualizar' : 'ProcedenciaAverias/Crear'
    saveData(url, objeto, load, close, dialogLoad)
}

// Funcion para Obtener los datos para editar
const obtenerElementoPorId = async (id) => {
    await obtener('ProcedenciaAverias/ObtenerPorId', id, objeto, dialogLoad, dialog)
}

// Funcion para eliminar elemento
const eliminar = async () => {
    await eliminarElemento(
        'ProcedenciaAverias/Eliminar',
...
const load = async () => {
    items.value = await loadGet('ProcedenciaAverias/ObtenerListadoPaginado')
}
