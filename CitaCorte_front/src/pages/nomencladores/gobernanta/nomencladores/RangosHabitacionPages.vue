<template>
    <div class="q-pa-xl">
        <q-breadcrumbs class="qb cursor-pointer q-pb-md">
            ...
            <q-breadcrumbs-el label="Rangos Habitación" />
        </q-breadcrumbs>
            ...
            <header class="q-pa-sm bg-primary">
                <q-toolbar>
                    <q-toolbar-title class="text-subtitle6 text-white">
                        {{
                            objeto?.id ? "Editar Rangos Habitación" : "Adicionar Rangos Habitación"
                        }}</q-toolbar-title>
                </q-toolbar>
            </header>
            <q-form @submit.prevent="Guardar()" @reset="close" ref="myForm">
                <div class="h row q-ma-md">
                    <q-input class="col-xs-12" autofocus label="Código" v-model="objeto.codigo" color="primary" counter
                        maxlength="10" :rules="[onlyLetter_Number, (val) =>
                            (items.length > 0
                    ...
                    <q-input class="col-xs-12 " label="Descripción*" v-model="objeto.descripcion" color="primary"
                        counter maxlength="40" :rules="[onlyLetter_Number, string, (val) =>
                            (items.length > 0
                                ? !isValorRepetido(val, 'descripcion', objeto, items)
                                : true) || 'Ya existe una descripción  con ese valor',]" />
                    <q-checkbox v-if="objeto.id && !objeto?.id" class="q-mr-md" right-label v-model="objeto.activo" label="Activo"
                        color="primary" />
...
<script setup>
// IMPORT LIBRERIES
import { dataColumnRanges_Rooms } from "src/assets/js/column_data/columnDataGobernanta";
import {
    loadGet,
    saveData,
    isValorRepetido,
...
// DATOS DE LA TABLA
const columns = dataColumnRanges_Rooms;
const filter = ref("");
const items = ref([]);
...
onMounted(async () => {
    ...
    items.value = (await loadGet('RangosHabitacion/ObtenerListadoPaginado')) ?? []; // Condicion para en caso de error la tabla no de error ya q la api devulve undefined
...
const Guardar = () => {
    const url = objeto.id ? 'RangosHabitacion/Actualizar' : 'RangosHabitacion/Crear'
    saveData(url, objeto, load, close, dialogLoad)
}

// Funcion para Obtener los datos para editar
const obtenerElementoPorId = async (id) => {
    await obtener('RangosHabitacion/ObtenerPorId', id, objeto, dialogLoad, dialog)
}

// Funcion para eliminar elemento
const eliminar = async () => {
    await eliminarElemento(
        'RangosHabitacion/Eliminar',
...
const load = async () => {
    items.value = await loadGet('RangosHabitacion/ObtenerListadoPaginado')
}
