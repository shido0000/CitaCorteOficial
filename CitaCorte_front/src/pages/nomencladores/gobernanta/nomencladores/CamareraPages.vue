<template>
    <div class="q-pa-xl">
        <q-breadcrumbs class="qb cursor-pointer q-pb-md">
            ...
            <q-breadcrumbs-el label="Camarera" />
        </q-breadcrumbs>
        ...
        <q-card style="width: 700px; max-width: 80vw; height: auto">
            <header class="q-pa-sm bg-primary">
                <q-toolbar>
                    <q-toolbar-title class="text-subtitle6 text-white">
                        {{
                            objeto?.id ? "Editar Camarera" : "Adicionar Camarera"
                        }}</q-toolbar-title>
                </q-toolbar>
            </header>
            <q-form @submit.prevent="Guardar()" @reset="close" ref="myForm">
                <div class="h row q-ma-md">
                    ...
                    <q-checkbox v-if="objeto.id" class="q-mr-md q-mt-sm" right-label v-model="objeto.supervisora"
                        label="Supervisora" color="primary" />
...
<script setup>
// IMPORT LIBRERIES
import { dataColumnCamarera } from "src/assets/js/column_data/columnDataGobernanta";
import {
    loadGet,
    saveData,
    isValorRepetido,
...
// DATOS DE LA TABLA
const columns = dataColumnCamarera;
const filter = ref("");
const items = ref([]);
...
onMounted(async () => {
    ...
    items.value = (await loadGet('Camarera/ObtenerListadoPaginado')) ?? []; // Condicion para en caso de error la tabla no de error ya q la api devulve undefined
...
const load = async () => {
    items.value = await loadGet('Camarera/ObtenerListadoPaginado')
}
