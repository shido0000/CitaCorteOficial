<template>
    <div class="q-pa-xl">
        <q-breadcrumbs class="qb cursor-pointer q-pb-md">
            ...
            <q-breadcrumbs-el label="Técnicos Averias" />
        </q-breadcrumbs>
            ...
            <header class="q-pa-sm bg-primary">
                <q-toolbar>
                    <q-toolbar-title class="text-subtitle6 text-white">
                        {{
                            objeto?.id ? "Editar Técnico Averia" : "Adicionar Técnico Averia"
                        }}</q-toolbar-title>
                </q-toolbar>
            </header>
            <q-form @submit.prevent="Guardar()" @reset="close" ref="myForm">
                <div class="h row q-ma-md">
                    <q-input class="col-xs-12 " label="Descripción*" v-model="objeto.descripcion" color="primary" autogrow
                        counter maxlength="100" :rules="[onlyLetter_Number, string, (val) =>
                            (items.length > 0
...
<script setup>
// IMPORT LIBRERIES
import { dataColumnTecnicosAveria } from "src/assets/js/column_data/columnDataGobernanta";
import {
    loadGet,
    saveData,
    isValorRepetido,
...
// DATOS DE LA TABLA
const columns = dataColumnTecnicosAveria;
const filter = ref("");
const items = ref([]);
...
onMounted(async () => {
    ...
    items.value = (await loadGet('TecnicosAverias/ObtenerListadoPaginado')) ?? []; // Condicion para en caso de error la tabla no de error ya q la api devulve undefined
...
const Guardar = () => {
    const url = objeto.id ? 'TecnicosAverias/Actualizar' : 'TecnicosAverias/Crear'
    saveData(url, objeto, load, close, dialogLoad)
}

// Funcion para Obtener los datos para editar
const obtenerElementoPorId = async (id) => {
    await obtener('TecnicosAverias/ObtenerPorId', id, objeto, dialogLoad, dialog)
}

// Funcion para eliminar elemento
const eliminar = async () => {
    await eliminarElemento(
        'TecnicosAverias/Eliminar',
...
const load = async () => {
    items.value = await loadGet('TecnicosAverias/ObtenerListadoPaginado')
}
