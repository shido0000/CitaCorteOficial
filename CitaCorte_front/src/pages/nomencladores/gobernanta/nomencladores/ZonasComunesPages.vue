<template>
    <div class="q-pa-xl">
        <q-breadcrumbs class="qb cursor-pointer q-pb-md">
            ...
            <q-breadcrumbs-el label="Zonas Comunes" />
        </q-breadcrumbs>
...
<script setup>
// IMPORT LIBRERIES
import { dataColumnZonasComunes } from "src/assets/js/column_data/columnDataGobernanta";
import {
    loadGet,
    saveData,
    isValorRepetido,
...
// DATOS DE LA TABLA
const columns = dataColumnZonasComunes;
const filter = ref("");
const items = ref([]);
...
onMounted(async () => {
    ...
    items.value = (await loadGet('ZonasComunes/ObtenerListadoPaginado')) ?? []; // Condicion para en caso de error la tabla no de error ya q la api devulve undefined
...
const Guardar = () => {
    const url = objeto.id ? 'ZonasComunes/Actualizar' : 'ZonasComunes/Crear'
    saveData(url, objeto, load, close, dialogLoad)
}

// Funcion para Obtener los datos para editar
const obtenerElementoPorId = async (id) => {
    await obtener('ZonasComunes/ObtenerPorId', id, objeto, dialogLoad, dialog)
}

// Funcion para eliminar elemento
const eliminar = async () => {
    await eliminarElemento(
        'ZonasComunes/Eliminar',
...
const load = async () => {
    items.value = await loadGet('ZonasComunes/ObtenerListadoPaginado')
}
