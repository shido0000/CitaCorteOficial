<template>
    <div class="q-pa-xl">
        <q-breadcrumbs class="qb cursor-pointer q-pb-md">
            ...
            <q-breadcrumbs-el label="Tipo Averia" />
        </q-breadcrumbs>
            ...
            <header class="q-pa-sm bg-primary">
                <q-toolbar>
                    <q-toolbar-title class="text-subtitle6 text-white">
                        {{
                            objeto?.id ? "Editar Tipo Averia" : "Adicionar Tipo Averia"
                        }}</q-toolbar-title>
                </q-toolbar>
            </header>
...
<script setup>
// IMPORT LIBRERIES
import { dataColumnTipoAveria } from "src/assets/js/column_data/columnDataGobernanta";
import {
    loadGet,
    saveData,
    isValorRepetido,
...
// DATOS DE LA TABLA
const columns = dataColumnTipoAveria;
const filter = ref("");
const items = ref([]);
...
onMounted(async () => {
    ...
    items.value = (await loadGet('TiposAverias/ObtenerListadoPaginado')) ?? []; // Condicion para en caso de error la tabla no de error ya q la api devulve undefined
...
const Guardar = () => {
    const url = objeto.id ? 'TiposAverias/Actualizar' : 'TiposAverias/Crear'
    console.log(objeto)
...
const obtenerElementoPorId = async (id) => {
    await obtener('TiposAverias/ObtenerPorId', id, objeto, dialogLoad, dialog)
}

// Funcion para eliminar elemento
const eliminar = async () => {
    await eliminarElemento(
        'TiposAverias/Eliminar',
...
const load = async () => {
    items.value = await loadGet('TiposAverias/ObtenerListadoPaginado')
}
