<template>
    <q-page padding>
        <div class="row items-center q-mb-md">
            <div class="text-h6">Servicios de la Barbería</div>
            <q-space />
...
import { useQuasar, Dialog } from 'quasar'   // 👈 Importa Dialog
import { api } from 'src/boot/axios'
import { decodeToken } from 'src/stores/auth'

// const $q = useQuasar()  // ya no es necesario para diálogos, pero lo dejamos para notificaciones
const $q = useQuasar()
const servicios = ref([])
...
const obtenerBarberiaId = () => {
    ...
    return decoded?.barberiaId || null
}

const cargar = async () => {
    loading.value = true
    try {
        const barberiaId = obtenerBarberiaId()
        if (!barberiaId) throw new Error('No se encontró barberiaId en el token')
        const { data } = await api.get(`Servicio/ObtenerListadoPaginado?barberiaId=${barberiaId}`)
...
const abrirFormulario = (row = null) => {
    if (row) {
        ...
        form.value = {
            ...
            barberiaId: row.barberiaId || obtenerBarberiaId(),
            barberoId: null
        }
